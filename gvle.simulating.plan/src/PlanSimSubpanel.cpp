...
void
PlanSimSubpanelThread::onStarted()
{
...
PlanSimSubpanelLeftWidget::PlanSimSubpanelLeftWidget()
...
PlanSimSubpanelRightWidget::PlanSimSubpanelRightWidget()
...
QString
PlanSimSubpanel::getname()
{
    return "Plan";
}

QWidget*
PlanSimSubpanel::leftWidget()
{
    return left;
}

QWidget*
PlanSimSubpanel::rightWidget()
{
    return right;
}

void
PlanSimSubpanel::undo()
{
}

void
PlanSimSubpanel::redo()
{
}

PluginSimPanel*
PlanSimSubpanel::newInstance()
{
    return new PlanSimSubpanel;
}

void
PlanSimSubpanel::showCustomPlot(bool b)
{
    ...
    if (b) {
        left->ui->stackPlot->addWidget(left->planView);
...
portToPlot*
PlanSimSubpanel::getPortToPlot(QString view, QString port)
{
    std::vector<portToPlot>::iterator itb = portsToPlot.begin();
    std::vector<portToPlot>::iterator ite = portsToPlot.end();
    std::vector<portToPlot>::iterator itf = ite;
...
void
PlanSimSubpanel::updateCustomPlot()
{
    // get results
    ...
    for (; itb != ite; itb++) {
        ...
        mPlan.loadString(getString(view, 1, 1, false));

        if (mPlan.isValid())
            left->planView->loadPlan(&mPlan);
        else
        {
            QMessageBox msg;
...
void
PlanSimSubpanel::onDebugChanged(QString val)
{
    if (val == "true") {
        debug = true;
    } else {
...
void
PlanSimSubpanel::onThreadsChanged(int val)
{
    if (val > 0) {
...
void
PlanSimSubpanel::onBlockSizeChanged(int val)
{
    blockSize = val;
}

void
PlanSimSubpanel::onTreeItemChanged(QTreeWidgetItem* item, int col)
{
