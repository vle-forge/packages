...
void Plugin::generate(vpz::AtomicModel& model,
                      vpz::Dynamic& dynamic,
...
{
    generatePorts(model);
    generateCondition(model, conditions);
...
void Plugin::generateInputPorts(vpz::AtomicModel& model)
{
    if (mOldMode != mMapping.getMode()) {
        if (mOldMode == "name") {
            if (model.existInputPort("update")) {
                model.delInputPort("update");
            }
        } else if (mOldMode == "port") {
            for (std::vector < std::string >::const_iterator it =
                     mOldExternalVariables.begin();
                 it != mOldExternalVariables.end(); ++it) {
                if (model.existInputPort(*it)) {
    ...
    if (mMapping.getMode() == "name") {
        if (not model.existInputPort("update")) {
            model.addInputPort("update");
        }
        ...
        for (Parameters::ExternalVariables_t::const_iterator it =
             externalVariables.begin(); it != externalVariables.end(); ++it) {
            if (not model.existInputPort(it->first)) {
                model.addInputPort(it->first);
            }
        }
...
void Plugin::generatePorts(vpz::AtomicModel& model)
{
    generateInputPorts(model);
    generateOutputPorts(model);
}
