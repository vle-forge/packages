...
void Statechart::resizeState(State* state, int width, int height)
{
    ...
    double ratiox = (double)width / state->width();
    double ratioy = (double)height / state->height();

    while (it != mTransitions.end()) {
        if ((*it)->source() == state->name()) {
            (*it)->replaceSource(state->x(), state->y(),
                                 ratiox, ratioy);
        }
        // to check the case of the self transition
        if ((*it)->points().back() != (*it)->points().front())  {
            if ((*it)->destination() == state->name()) {
                (*it)->replaceDestination(state->x(), state->y(),
                                          ratiox, ratioy);
            }
        }
        ++it;
...
void Statechart::changeStateNameTransitions(const std::string& oldName,
